...
from .utils import partition_num,alias_sample, create_alias_table
...
class RandomWalker:
    ...
    def simulate_walks(self, num_walks, walk_length, workers=1, verbose=0):
        """
        该函数实现了游走采样，在num_walks轮内，每一轮对所有数据进行一次游走，游走方法有两种，根据 p q 参数控制，如果是多cpu并行，总轮数和不变
        :param num_walks: 采样总轮数，相当于epochs
    ...
    def _simulate_walks(self, nodes, num_walks, walk_length,):
        ...
        :param nodes: 图中所有节点
        :param num_walks: 一个Walker处理的轮数，相当于epochs
        :param walk_length: 节点游走长度
        :return: 返回值是一个长度为num_walks * len(nodes) 的list1，
    ...
    def _exec_random_walk(self, graphs, layers_accept, layers_alias, v, walk_length, gamma, stay_prob=0.3):
        ...
        while len(path) < walk_length:
            ...
            if(r < stay_prob):  # same layer
                v = _chooseNeighbor(v, graphs, layers_alias,
                                   layers_accept, layer)
                path.append(self.idx2node[v])
